      ...
      <div className="bg-slate-950 w-full max-w-6xl rounded-[2rem] md:rounded-[3rem] shadow-2xl border border-slate-800 animate-in zoom-in-95 h-[92vh] flex flex-col text-white">
        <div className="flex flex-col md:flex-row h-full text-white overflow-hidden">
          <div className="w-full md:w-[35%] bg-black p-6 md:p-14 border-b md:border-b-0 md:border-r border-slate-900 flex flex-col items-center text-center text-white overflow-y-auto custom-scrollbar">
            ...
            <div className="bg-slate-900 border px-5 py-2.5 rounded-xl mb-8 flex items-center gap-2 text-white"><CalendarCheck size={14} className="text-emerald-400" /><span className="text-[11px] font-black text-slate-300 uppercase italic leading-none">Última Visita: {stats.lastVisitDate}</span></div>

            <div className="w-full bg-slate-900/60 p-6 rounded-[2rem] border border-indigo-500/20 mb-8 text-white">
              <div className="flex justify-between items-center mb-4 text-white"><span className="text-[10px] font-black text-indigo-400 uppercase italic tracking-widest flex items-center gap-2 leading-none"><Gift size={14} /> PROGRESO VIP</span><span className="text-[10px] font-black text-white italic leading-none">{stats.progressTowardReward} / {stats.rewardVisitsGoal}</span></div>
              <div className="flex gap-2 h-3 bg-black rounded-full overflow-hidden p-0.5 text-white">{[...Array(stats.rewardVisitsGoal)].map((_, index) => (<div key={index} className={`flex-1 rounded-full ${index < stats.progressTowardReward ? 'bg-indigo-500' : 'bg-slate-800'}`}></div>))}</div>
            </div>

            <div className="grid grid-cols-2 gap-4 w-full text-white mb-8">
              <div className="bg-slate-900 p-5 rounded-[2rem] flex flex-col items-center justify-center text-white border border-white/5"><p className="text-[9px] font-black text-slate-500 uppercase italic leading-none">VISITAS</p><h4 className="text-3xl font-black italic text-white leading-none mt-1">{stats.totalVisits}</h4></div>
              <div className="bg-slate-900 p-5 rounded-[2rem] flex flex-col items-center justify-center text-white border border-white/5"><p className="text-[9px] font-black text-slate-500 uppercase italic leading-none">GASTADO</p><h4 className="text-xl font-black text-emerald-400 italic mt-1 leading-none">C$ {stats.totalSpent}</h4></div>
            </div>
          </div>
          ...
          <div className="flex-1 p-6 md:p-14 flex flex-col bg-slate-950/50 text-white overflow-hidden text-white">
            ...
            <div className="flex-1 space-y-6 overflow-y-auto custom-scrollbar pr-2">
              <div className="space-y-4">
                <div className="bg-slate-900/40 border border-slate-800 p-6 rounded-[2.5rem] flex items-center justify-between group hover:border-indigo-500/30 transition-all">
                  <div className="flex items-center gap-6">
                    ...
                    <span className="text-xs font-black text-slate-400 uppercase italic tracking-[0.2em]">Servicio Favorito</span>
                  </div>
                  <span className="text-lg font-black text-white uppercase italic tracking-tighter">{stats.favService}</span>
                </div>

                <div className="bg-slate-900/40 border border-amber-500/20 p-6 rounded-[2.5rem] flex items-center justify-between group hover:border-amber-500/40 transition-all">
                  <div className="flex items-center gap-6">
                    ...
                    <span className="text-xs font-black text-slate-400 uppercase italic tracking-[0.2em]">Barbero Favorito</span>
                  </div>
                  <span className="text-lg font-black text-white uppercase italic tracking-tighter">{stats.favBarber}</span>
                </div>
              </div>
              ...
              <div className="pt-6 space-y-6">
                ...
                <div className="bg-black/40 border border-white/5 p-10 rounded-[2.5rem] shadow-inner relative overflow-hidden group min-h-[180px]">
                  <div className="absolute top-0 left-0 w-1.5 h-full bg-indigo-600"></div>
                  <p className="italic text-white text-lg leading-relaxed text-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5 mt-8 md:mt-10">
              ...
              <button onClick={onEdit} className="bg-slate-800 hover:bg-slate-700 text-white py-6 rounded-2xl font-black uppercase italic text-[11px] transition-all flex items-center justify-center gap-3 text-white">
