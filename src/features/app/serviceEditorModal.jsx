...
export function ServiceEditorModal({ services, inventoryItems = [], serviceCategoryOptions = CATEGORIES, onClose, onSave, initial }) {
  ...
  return (
    <div className="barber-service-editor fixed inset-0 z-[300] flex items-center justify-center bg-black/82 p-3 backdrop-blur-xl animate-in fade-in text-white no-print">
      <div className="barber-service-panel relative flex h-[88vh] w-[92vw] max-w-[1500px] flex-col overflow-hidden rounded-[1.6rem] border border-cyan-300/35 bg-slate-950 shadow-[0_30px_110px_rgba(0,0,0,0.65)] animate-in zoom-in-95">
        <div className="flex items-center justify-between gap-4 border-b border-cyan-300/20 bg-slate-900/95 px-5 py-4 md:px-8">
          <div className="flex items-center gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-600 to-cyan-400 text-white shadow-[0_14px_34px_rgba(34,211,238,0.22)]">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-300">Catálogo de barbería</p>
              <h3 className="mt-1 text-2xl font-black uppercase italic tracking-tighter leading-none text-white">{initial?.id ? 'Editar servicio' : 'Nuevo servicio'}</h3>
            </div>
          </div>
          <button onClick={onClose} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-cyan-300/35 bg-slate-950 text-cyan-200 transition-all hover:border-cyan-200 hover:bg-cyan-300/10">
            <X size={20} />
          </button>
        </div>
          ...
          <div className="grid min-h-full grid-cols-1 gap-4 p-4 pb-24 lg:p-6 lg:pb-24">
            <div className={isCombo ? "grid min-h-full grid-cols-1 gap-4 xl:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)]" : "grid grid-cols-1 gap-4"}>
              <section className={isCombo ? "flex min-h-0 flex-col rounded-[1.35rem] border border-cyan-300/30 bg-slate-900/78 p-4 xl:min-h-[calc(88vh-12rem)]" : "rounded-[1.35rem] border border-cyan-300/30 bg-slate-900/78 p-4"}>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <label className="space-y-2">
                    ...
                    <select
                      className="w-full rounded-2xl border border-cyan-300/30 bg-black px-5 py-4 text-sm font-black uppercase italic text-white outline-none focus:border-cyan-300"
                  ...
                  <label className="space-y-2">
                    <span className="block text-[9px] font-black uppercase italic tracking-[0.18em] text-slate-400">Nombre</span>
                    <input
                      required
                ...
                {!isPromotion ? (
                  <label className="mt-5 block space-y-2">
                    <span className="block text-[9px] font-black uppercase italic tracking-[0.18em] text-slate-400">Precio final (C$)</span>
                    <div className="relative">
                      <input
                        required
                        type="number"
                        className="w-full rounded-[1.8rem] border border-cyan-300/30 bg-black py-5 pl-20 pr-8 text-4xl font-black italic leading-none text-white outline-none focus:border-cyan-300"
              ...
              {isPromotion ? (
                <section className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] rounded-[1.8rem] border border-[#b7d8c7] bg-[#edf7f2] p-5">
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <label className="space-y-2">
                      <span className="block text-[9px] font-black uppercase italic tracking-[0.18em] text-cyan-300">Cobertura</span>
                    ...
                    <label className="space-y-2">
                      <span className="block text-[9px] font-black uppercase italic tracking-[0.18em] text-cyan-300">Tipo de descuento</span>
                      <select
                  ...
                  <label className="block space-y-2">
                    <span className="block text-[9px] font-black uppercase italic tracking-[0.18em] text-cyan-300">
              ...
              {isCombo ? (
                <section className="flex min-h-0 flex-col gap-4 rounded-[1.35rem] border border-cyan-300/30 bg-slate-900/78 p-4 xl:min-h-[calc(88vh-12rem)]">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-300">Componer combo</p>
                      <p className="mt-1 text-[11px] font-bold text-slate-400">Selecciona servicios para calcular el precio base.</p>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-right">
                      <div className="rounded-2xl border border-cyan-300/25 bg-black/45 px-3 py-2">
                        <p className="text-[8px] font-black uppercase tracking-[0.16em] text-slate-500">Items</p>
                        <p className="text-sm font-black italic text-cyan-200">{formData.items.length}</p>
                      </div>
                      <div className="rounded-2xl border border-cyan-300/25 bg-black/45 px-3 py-2">
                        <p className="text-[8px] font-black uppercase tracking-[0.16em] text-slate-500">Base</p>
                        <p className="text-sm font-black italic text-cyan-200">C$ {Number(formData.price || 0).toLocaleString('es-NI')}</p>
                      </div>
                    </div>
                  </div>
                  <input
                    ...
                    className="w-full rounded-2xl border border-cyan-300/30 bg-black px-5 py-4 text-xs font-black uppercase italic text-white outline-none focus:border-cyan-300"
                    value={searchTerm}
              ...
              {canConfigureSupplies && isSupplyConfigOpen ? (
                <section className="flex min-h-0 flex-col gap-4 rounded-[1.35rem] border border-cyan-300/30 bg-slate-900/78 p-4 xl:min-h-[calc(88vh-12rem)]">
                  <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-300">Insumos del servicio</p>
                      <p className="mt-1 text-[11px] font-bold text-slate-400">Configura lo que se descontará del inventario al cobrar.</p>
                    </div>
                    <div className="rounded-2xl border border-[#b7d8c7] bg-white px-4 py-3">
                      <p className="text-[8px] font-black uppercase tracking-[0.16em] text-cyan-300">Costo estimado</p>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] gap-4">
                    <div className="space-y-3 rounded-[1.5rem] border border-[#f2c1d4] bg-white p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="rounded-full border border-[#b7d8c7] bg-[#edf7f2] px-3 py-1 text-[8px] font-black uppercase text-cyan-300">
                          {supplyItems.length}
                        </span>
                      </div>
                      <input
                      ...
                      <div className="space-y-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                        {supplyItems.map((item) => (
                          ...
                          >
                            <div className="min-w-0">
                              <p className="truncate text-[11px] font-black uppercase italic text-white">{item.productName || item.name}</p>
                        ...
                        {!supplyItems.length && (
                          <div className="rounded-2xl border border-dashed border-[#ee9fbc] bg-white p-5 text-center text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">
                    ...
                    <div className="space-y-3 rounded-[1.5rem] border border-[#f2c1d4] bg-white p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="rounded-full border border-[#ee9fbc] bg-[#fff7fb] px-3 py-1 text-[8px] font-black uppercase text-slate-400">
                      ...
                      {!formData.inventoryUsage.length && (
                        <div className="rounded-2xl border border-dashed border-[#ee9fbc] bg-white p-8 text-center text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">
            ...
            <aside className="space-y-4">
              <div className="rounded-[1.8rem] border border-[#b7d8c7] bg-[#edf7f2] p-5">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-300">Resumen</p>
                <h4 className="mt-3 text-lg font-black uppercase italic leading-tight text-white">{formData.name || 'Servicio sin nombre'}</h4>
                <div className="mt-5 space-y-3 text-sm font-black">
                  <div className="flex justify-between gap-3 border-b border-[#b7d8c7] pb-3">
                    ...
                    <span className="text-white">C$ {Number(formData.price || 0).toLocaleString('es-NI')}</span>
                  </div>
                  <div className="flex justify-between gap-3 border-b border-[#b7d8c7] pb-3">
                    <span className="text-[#856a75]">Insumos</span>
                    <span className="text-cyan-300">C$ {supplyCost.toLocaleString('es-NI')}</span>
                  </div>
              ...
              <button type="button" onClick={onClose} className="w-full rounded-[1.6rem] border border-[#ee9fbc] bg-white px-6 py-4 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">
                Cancelar
              </button>
            </aside>
            )}
        ...
        <div className="absolute inset-x-0 bottom-0 z-20 flex flex-col justify-end gap-3 border-t border-cyan-300/20 bg-slate-950/95 p-4 backdrop-blur sm:flex-row">
          <button type="submit" form="service-editor-form" className="rounded-[1.2rem] bg-gradient-to-r from-indigo-600 to-cyan-400 px-7 py-4 text-[11px] font-black uppercase italic tracking-[0.16em] text-white shadow-[0_16px_34px_rgba(34,211,238,0.18)] transition-all hover:-translate-y-0.5 hover:shadow-[0_20px_44px_rgba(34,211,238,0.25)]">
            Guardar catálogo
          </button>
          <button type="button" onClick={onClose} className="rounded-[1.2rem] border border-cyan-300/30 bg-slate-950 px-7 py-4 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-200 transition-all hover:bg-cyan-300/10">
            Cancelar
          </button>
        </div>
        ...
        {canConfigureSupplies && (
          <button
            ...
            className="absolute bottom-4 left-4 z-30 flex items-center gap-3 rounded-[1.4rem] border border-[#8fd6cf] bg-[#2fb7ae] px-5 py-4 text-left text-white shadow-[0_16px_34px_rgba(47,183,174,0.28)] transition-all hover:bg-[#259f98] active:scale-95"
          >
            <Package size={18} />
            <span>
            ...
            <div className="barber-supply-panel flex h-[94vh] w-[98vw] max-w-[1600px] flex-col overflow-hidden rounded-[1.6rem] border border-[#8fd6cf] bg-white shadow-[0_30px_90px_rgba(23,91,86,0.32)] animate-in zoom-in-95">
              <div className="flex items-center justify-between gap-4 border-b border-[#c6ebe6] bg-[#ecfbf8] px-5 md:px-7 py-3">
                <div>
                  ...
                  <h3 className="mt-1 text-xl font-black uppercase italic tracking-tighter leading-none text-white">
              ...
              <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden p-4 md:p-5">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <div className="flex items-center gap-2 rounded-2xl border border-[#8fd6cf] bg-white px-3 py-2 focus-within:border-[#2fb7ae]">
                      ...
                      <input
                        ...
                        className="min-w-0 flex-1 bg-transparent text-xs font-black uppercase italic text-white outline-none placeholder:text-[#9ab8b4]"
                        value={supplySearch}
                        onChange={(event) => setSupplySearch(event.target.value)}
                      />
                      {supplySearch && (
                        <button
                          ...
                          className="flex h-7 w-7 items-center justify-center rounded-full text-slate-400 hover:bg-[#fff7fb]"
                ...
                <section className="flex min-h-0 flex-col space-y-3 rounded-[1.35rem] border border-[#ee9fbc] bg-[#fff7fb] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-300">Seleccionados</p>
                      <p className="mt-1 text-[11px] font-bold text-slate-400">Cantidad estandar que se descontara por servicio.</p>
                    </div>
                  ...
                  <div className="min-h-0 min-h-0 flex-1 overflow-y-auto custom-scrollbar">
                    {formData.inventoryUsage.length > 0 && (
                      <div className="rounded-2xl border border-[#ee9fbc] bg-white">
                        <div>
                          <div className="grid grid-cols-[minmax(0,1fr)_120px_90px_115px_50px] items-center gap-3 border-b border-[#f2c1d4] bg-[#fff7fb] px-3 py-2 text-[8px] font-black uppercase tracking-[0.14em] text-slate-400">
                            <span>Producto</span>
                            <span>Cantidad</span>
                            <span>Unidad</span>
                    ...
                    {!formData.inventoryUsage.length && (
                      <div className="rounded-2xl border border-dashed border-[#ee9fbc] bg-white p-10 text-center text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">
                          Este servicio todavía no tiene insumos
                      </div>
                    )}
                  </div>
                    ...
                    {formData.inventoryUsage.map((usage) => {
                      ...
                      return (
                        <div key={usage.inventoryItemId} className="rounded-2xl border border-[#ee9fbc] bg-white p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <p className="truncate text-[12px] font-black uppercase italic text-white">{item?.productName || item?.name || 'Insumo'}</p>
                ...
                <section className="rounded-[1.25rem] border border-[#c6ebe6] bg-[#f8fffd] p-4">
                  <div className="flex flex-col gap-2 border-b border-[#c6ebe6] pb-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <h4 className="mt-1 text-base font-black uppercase italic text-white">{formData.name || 'Servicio sin nombre'}</h4>
                    </div>
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
                    ...
                    <div className="flex items-center justify-between gap-4 border-t border-[#c6ebe6] pt-3 text-base font-black md:border-l md:border-t-0 md:pl-4 md:pt-0">
                      <span className="uppercase tracking-[0.14em] text-slate-400">Margen bruto estimado</span>
