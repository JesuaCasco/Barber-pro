...
-- BarberPro tenia una mezcla de caja legacy (movement_type) y caja nueva
-- importada desde SalonPro (movement_kind). Esta migracion deja ambas rutas
-- compatibles para que los RPC y los inserts directos no fallen por columnas
-- obligatorias antiguas.
...
alter table if exists public.cash_sessions
  ...
  add column if not exists expected_cash_amount numeric not null default 0,
  add column if not exists counted_cash_amount numeric;

alter table if exists public.cash_sessions
  alter column status set default 'open',
  alter column expected_amount set default 0,
  alter column expected_cash_amount set default 0;
...
set movement_kind = coalesce(
  movement_kind,
  case movement_type
    when 'apertura' then 'opening'
    when 'venta' then 'sale'
    when 'cierre' then 'closing_adjustment'
    else 'manual'
  end,
  'manual'
);
...
set type = coalesce(
  type,
  case
    when movement_type in ('gasto', 'retiro') then 'out'
    when movement_kind = 'payroll_payment' then 'out'
    else 'in'
  end
);
...
set movement_type = coalesce(
  movement_type,
  case
    when movement_kind = 'opening' then 'apertura'
    when movement_kind = 'sale' then 'venta'
    when movement_kind = 'payroll_payment' then 'retiro'
    when movement_kind = 'closing_adjustment' then 'cierre'
    when type = 'out' then 'retiro'
    else 'ingreso_manual'
  end
);
...
alter table if exists public.cash_movements
  alter column movement_type set default 'ingreso_manual',
  alter column movement_kind set default 'manual',
...
alter table if exists public.cash_movements
  ...
  check (movement_type in ('apertura', 'venta', 'gasto', 'ingreso_manual', 'retiro', 'cierre', 'propina'));

create or replace function public.sync_cash_movement_legacy_fields()
returns trigger
language plpgsql
security definer
...
as $$
begin
  new.movement_kind := coalesce(nullif(new.movement_kind, ''), 'manual');
  new.payment_method := coalesce(nullif(new.payment_method, ''), 'cash');
  new.amount := coalesce(new.amount, 0);

  if new.type is null then
    new.type := case
      when new.movement_kind = 'payroll_payment' then 'out'
      else 'in'
    end;
  end if;
  ...
  if new.movement_type is null
     or (
       new.movement_type = 'ingreso_manual'
       and new.movement_kind in ('opening', 'sale', 'payroll_payment', 'closing_adjustment')
     ) then
    new.movement_type := case
      when new.movement_kind = 'opening' then 'apertura'
      when new.movement_kind = 'sale' then 'venta'
      when new.movement_kind = 'payroll_payment' then 'retiro'
      when new.movement_kind = 'closing_adjustment' then 'cierre'
      when new.type = 'out' then 'retiro'
      else 'ingreso_manual'
    end;
  end if;
...
drop trigger if exists trg_sync_cash_movement_legacy_fields on public.cash_movements;
create trigger trg_sync_cash_movement_legacy_fields
before insert or update on public.cash_movements
for each row
execute function public.sync_cash_movement_legacy_fields();
...
begin
  if v_auth_user is null then
    raise exception 'Debes iniciar sesion para abrir caja.';
  end if;

  if not public.cash_user_can_access_barbershop(v_auth_user, p_barbershop_id) then
    raise exception 'No tienes permiso para abrir caja en esta barberia.';
  end if;
...
begin
  ...
  if v_barbershop_id is null then
    raise exception 'No se pudo resolver la barberia para abrir caja desde la sucursal.';
  end if;
...
begin
  ...
  if not public.cash_user_can_access_barbershop(v_auth_user, p_barbershop_id) then
    raise exception 'No tienes permiso para cerrar caja en esta barberia.';
  end if;
...
begin
  ...
  if not public.cash_user_can_access_barbershop(v_auth_user, p_barbershop_id) then
    raise exception 'No tienes permiso para registrar ventas en esta barberia.';
  end if;
...
grant execute on function public.sync_cash_movement_legacy_fields() to authenticated;
